...
const STORAGE_DIR = 'line-data'
const MESSAGES_FILE = 'messages.json'

/**
 * Line message storage
 * Single-user mode: stores all messages in a flat list
 */
class LineStorage {
  private storagePath: string
  private messages: StoredLineMessage[] = []
  ...
  private async loadData(): Promise<void> {
    try {
      ...
      if (Array.isArray(data)) {
        this.messages = data as StoredLineMessage[]
        console.log('[Line Storage] Loaded', this.messages.length, 'messages')
      } else {
        console.log('[Line Storage] Invalid data format, starting fresh')
        this.messages = []
      }
    } catch {
      this.messages = []
      console.log('[Line Storage] No existing messages found')
    }
  }
   ...
   * Store a message
   */
  async storeMessage(message: StoredLineMessage): Promise<void> {
   ...
   */
  async getMessages(limit?: number): Promise<StoredLineMessage[]> {
    await this.ensureInitialized()
...
export const lineStorage = new LineStorage()
