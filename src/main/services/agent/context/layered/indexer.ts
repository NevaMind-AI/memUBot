...
export class LayeredContextIndexer {
  ...
  async buildIndex(input: LayeredIndexBuildInput): Promise<LayeredIndexBuildResult> {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const overview = await this.summaryGenerator.generateOverview(transcript, input.config.l1TargetTokens)
      const abstract = await this.summaryGenerator.generateAbstract(overview.text, input.config.l0TargetTokens)

      if (overview.fallbackUsed && overview.fallbackReason) {
        fallbackEvents.push(overview.fallbackReason)
      }
      ...
      const fullContentPath = await this.storage.writeArchive(input.sessionKey, nodeId, {
      ...
      nextNodes.push({
        ...
        tokenEstimate: {
          ...
          l1: estimateTextTokens(overview.text),
    ...
    const rootOverviewSource = kept
      .map((node) => `Archive ${node.id}\n${node.overview}`)
      .join('\n\n')

    const rootOverview = await this.summaryGenerator.generateOverview(
      rootOverviewSource || 'No archived context is available.',
      input.config.l1TargetTokens
    )
    const rootAbstract = await this.summaryGenerator.generateAbstract(rootOverview.text, input.config.l0TargetTokens)

    if (rootOverview.fallbackUsed && rootOverview.fallbackReason) {
      fallbackEvents.push(rootOverview.fallbackReason)
    }
