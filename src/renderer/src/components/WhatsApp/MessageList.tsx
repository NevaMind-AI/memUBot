...
export function WhatsAppMessageList(): JSX.Element {
  ...
  useEffect(() => {
    const unsubscribeMessages = window.whatsapp.onNewMessage((message: AppMessage) => {
      setMessages((prev) => {
    ...
    const unsubscribeStatus = window.whatsapp.onStatusChanged((status: BotStatus) => {
  ...
  const loadMessages = async () => {
    setLoading(true)
    try {
      const result = await window.whatsapp.getMessages(200)
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="w-6 h-6 border-2 border-[#25D366]/50 border-t-transparent rounded-full animate-spin" />
      </div>
    )
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-[#25D366]/10 border border-[#25D366]/20 shadow-lg flex items-center justify-center">
          ...
          <p className="text-[13px] text-[var(--text-muted)] max-w-[220px]">
            Connect to WhatsApp to start chatting.
          </p>
        </div>
      </div>
    )
        ...
        {groupedMessages.map((group, groupIndex) => (
          <div key={groupIndex}>
            ...
            <div className="flex items-center justify-center mb-4">
              <div className="px-3 py-1.5 rounded-full bg-[#25D366]/10 border border-[#25D366]/20 shadow-sm text-[11px] text-[#25D366] font-medium">
                {group.date}
              </div>
            </div>
                  ...
                  {msg.isFromBot ? (
                    botAvatarUrl ? (
                      <img
                        src={botAvatarUrl}
                        alt="Bot"
                        className="w-8 h-8 rounded-xl flex-shrink-0 object-cover border-2 border-[#25D366] shadow-md"
                      />
                    ) : (
                      <div className="w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center shadow-md bg-gradient-to-br from-[#25D366] to-[#128C7E]">
                  ...
                  <div className={`max-w-[70%] ${msg.isFromBot ? 'items-end' : 'items-start'}`}>
                    ...
                    <div
                      className={`px-4 py-2.5 rounded-2xl shadow-sm ${
                        msg.isFromBot
                          ? 'bg-gradient-to-br from-[#25D366]/20 to-[#128C7E]/20 backdrop-blur-sm border border-[#25D366]/30 rounded-tr-md'
