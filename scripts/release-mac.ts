...
function loadConfig(): ReleaseConfig {
  ...
  // S3 prefix for auto-update metadata (latest-mac.yml + installer)
 ...
 * electron-builder generates latest-mac.yml alongside the DMG in dist/.
...
async function uploadUpdateMetadata(
  ...
  // Find latest-mac.yml (auto-generated by electron-builder)
  const ymlPath = path.join(distDir, 'latest-mac.yml')
  try {
    await fs.stat(ymlPath)
  } catch {
    console.warn('[Update] latest-mac.yml not found — skipping update metadata upload')
    return uploadedKeys
  }

  // Upload latest-mac.yml
  const ymlKey = `${prefix}/latest-mac.yml`
  await client.send(
  ...
  // Upload the DMG to the updates prefix (electron-updater downloads from here)
  ...
  uploadedKeys.push(dmgKey)
  console.log(`[Update] Uploaded s3://${cfg.s3Bucket}/${dmgKey}`)
...
async function main(): Promise<void> {
  ...
  const updateKeys = await uploadUpdateMetadata(distDir, finalDmgPath, cfg)
  if (updateKeys.length > 0) {
    console.log(`- Uploaded ${updateKeys.length} update file(s)`)
  } else {
    console.log('- Skipped (no latest-mac.yml found)')
  }
